...
/// Floating voice indicator — trail orb at top-center. Never becomes key
/// (holding PTT must not steal scroll from the front app).
class FloatingBarController {
    ...
    @MainActor func show() {
        ...
        if let screen = NSScreen.main {
            win.setFrame(OverlayPanel.topCenteredFrame(size: size, visibleFrame: screen.visibleFrame), display: true)
        }
